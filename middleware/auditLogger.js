...
const auditLogger = async (req, res, next) => {
  if (["POST", "PUT", "DELETE"].includes(req.method)) {
    res.on("finish", async () => {
      if (res.statusCode >= 200 && res.statusCode < 300) {
        try {
          // Sanitize to remove sensitive fields like passwords
          ...
          const log = new AuditLog({
            user: {
                // Now uses the exact role from your list: admin, teacher, student, etc.
                username: req.user?.username || "System_User",
                role: req.user?.role || "clerk" 
            },
            action: `${req.method}_${req.path.split('/')[1]?.toUpperCase() || 'ACTION'}`,
            method: req.method,
            endpoint: req.originalUrl,
