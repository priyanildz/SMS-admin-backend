...
// ----------------------------------------------------
// Email Helper Function (NodeMailer setup placeholder)
// ----------------------------------------------------

const sendAdmissionEmail = async (studentName, recipientEmail, loginId, password) => {
    // 🚨 IMPORTANT: Replace this with your actual email service configuration.
    // Ensure EMAIL_USER and EMAIL_PASS are set in your environment variables.
    
    // Example SMTP setup (e.g., using Gmail with an App Password)
    ...
    const mailOptions = {
        from: `"${process.env.SCHOOL_NAME || 'SSPD School'}" <${transporter.options.auth.user}>`,
        to: recipientEmail,
        subject: '🚀 Your Student Admission Confirmation and Login Details',
        html: `
            <p>Dear ${studentName},</p>
            <p>Congratulations! Your admission application has been successfully processed.</p>
            <p>You can now log in to the Student Management System using the following credentials:</p>
            <table style="border: 1px solid #ccc; border-collapse: collapse; margin: 15px 0;">
                <tr>
                    <td style="padding: 8px; border: 1px solid #ccc; background-color: #f2f2f2;"><strong>Login ID (Admission No.):</strong></td>
                    <td style="padding: 8px; border: 1px solid #ccc;">${loginId}</td>
                </tr>
                <tr>
                    <td style="padding: 8px; border: 1px solid #ccc; background-color: #f2f2f2;"><strong>Temporary Password (Date of Birth):</strong></td>
            ...
            <p>Thank you,<br>SSPD School Administration</p>
        `,
    };

    try {
        await transporter.sendMail(mailOptions);
        console.log(`Admission email successfully sent to ${recipientEmail}`);
        return true;
    } catch (error) {
        console.error(`Failed to send admission email to ${recipientEmail}:`, error);
        // Do NOT return false, as student creation was successful.
        return false;
    }
};
...
exports.createUser = async (req, res) => {
  try {
    ...
    // 🚨 NEW LOGIC: Send Admission Email after successful save
    const recipientEmail = user.parent.emailaddress; // Assuming email is stored in parent.emailaddress
    const loginId = user.admission.admissionno;      // Using Admission No. as Login ID
    
    // Format DOB as password (YYYY-MM-DD format)
    const rawDob = req.body.dob || user.dob;
    const password = new Date(rawDob).toISOString().split('T')[0];
    
    const studentName = `${user.firstname} ${user.lastname}`;
    
    // Execute email sending asynchronously (non-blocking)
    if (recipientEmail) {
        sendAdmissionEmail(studentName, recipientEmail, loginId, password);
    } else {
        console.warn(`No email address provided for student ${studentName}. Skipping email notification.`);
    }

    // Use 201 for resource creation
    res.status(201).json({ 
        message: "Student created successfully and login details sent via email." 
    });
  } catch (error) {
    console.error("Error creating student:", error);
