...
    if (std) query.std = std;
    if (div) query.div = div;
    // NOTE: This route needs the same robust search logic as filterTransactions
    if (search) {
        const searchTerms = search.trim().split(/\s+/);
        if (searchTerms.length > 1) {
            // Search for combinations like "First Last" or "Last First"
            query.$or = [
                { name: { $regex: searchTerms.join(' '), $options: "i" } },
                { name: { $regex: searchTerms.reverse().join(' '), $options: "i" } }
            ];
        } else {
            // Single term search (first name, last name, or part of full name)
...
    let query = {};
    ...
    // 1. Determine Standard Filter (Individual Standard OR Category Group)
    if (std) {
        query.std = std;
    } else if (category && category !== "All") {
        let standardList = [];
    ...
    // 2. Filter by Division and Search (Name)
    if (div) query.div = div;
    
    // FIX: Implement flexible First/Last Name Search
    if (search) {
        const searchTerms = search.trim().split(/\s+/);
        if (searchTerms.length > 1) {
            // Search for combinations like "First Last" or "Last First"
