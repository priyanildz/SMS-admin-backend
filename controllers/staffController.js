...
exports.getStaffTimetable = async (req, res) => {
  try {
    ...
    // 1. Fetch all timetables (or only those marked as 'published')
    const allTimetables = await Timetable.find({ status: 'published' });
    
    // 2. Map the FIXED_PERIOD_STRUCTURE to create a row-by-row view
    // This transforms the data from "Class-based" to "Teacher-based"
    const staffSchedule = FIXED_PERIOD_STRUCTURE.map(slot => {
      const row = { time: slot.time };
      
      WEEKDAYS.forEach(day => {
        const dayKey = day.substring(0, 3); // "Monday" -> "Mon"
        
        // Find if this specific teacher (staffId) has a class in ANY timetable during this slot
        const assignedSlot = allTimetables.find(tt => 
          tt.timetable.some(d => d.day === day && 
            d.periods.some(p => p.time === slot.time && p.teacher?.toString() === staffId)
          )
        );

        if (assignedSlot) {
          // Extract the specific subject name for this teacher
          const periodData = assignedSlot.timetable
            .find(d => d.day === day).periods
            .find(p => p.time === slot.time);

          row[dayKey] = `${periodData.subject} (Std ${assignedSlot.standard}-${assignedSlot.division})`;
        } else {
          // Handle non-teaching slots (Break/Lunch) or empty periods
          row[dayKey] = slot.type !== 'Period' ? slot.type : ""; 
        }
      });
    ...
    console.error("Error in getStaffTimetable:", error);
